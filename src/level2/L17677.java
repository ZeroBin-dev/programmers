...
/**
 * 제목 : 뉴스 클러스터링
 * 설명 : 여러 언론사에서 쏟아지는 뉴스, 특히 속보성 뉴스를 보면 비슷비슷한 제목의 기사가 많아 정작 필요한 기사를 찾기가 어렵다.
  ...
  public int solution(String str1, String str2) {
    ...
    String[] str1List =
      IntStream.range(0, str1.length() - 1)
        .mapToObj(i -> str1.substring(i, i + 2).toUpperCase())
        .filter(sub -> sub.chars().allMatch(Character::isLetter)) // 영문만 필터
        .toArray(String[]::new);

    String[] str2List =
      IntStream.range(0, str2.length() - 1)
        .mapToObj(i -> str2.substring(i, i + 2).toUpperCase())
        .filter(sub -> sub.chars().allMatch(Character::isLetter)) // 영문만 필터
        .toArray(String[]::new);

    if(str1List.length + str2List.length == 0){
      return t;
    }
    ...
    for(int i = 0 ; i < str1List.length ; i++){
      for(int j = 0 ; j < str2List.length ; j++){
        if(str1List[i].equals(str2List[j])){
          str1List[i] = "0";
          str2List[j] = "0";
          break;
        }
    ...
    // str1List 에서의 0의 갯수가 교집합 갯수
    int g = (int)Arrays.stream(str1List).filter(str -> str.equals("0")).count();

    // str1List 에서 0의 갯수를 빼고 + str2List 의 length 를 하면 합집합
    int h = str1List.length - g + str2List.length;

    // 교집합은 0 인데 합집합은 0이 아닐때 0을 리턴?
    if(g == 0 && h != 0){
      return 0;
    }

    return (int)(((double)g/h)*t);
  }
