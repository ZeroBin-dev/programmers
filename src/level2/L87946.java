...
/**
 * 제목 : 피로도
 * 설명 : XX게임에는 피로도 시스템(0 이상의 정수로 표현합니다)이 있으며,
 ...
 * "소모 피로도"는 던전을 탐험한 후 소모되는 피로도를 나타냅니다. 예를 들어 "최소 필요 피로도"가 80, "소모 피로도"가 20인 던전을 탐험하기 위해서는 유저의 현재 남은 피로도는 80 이상 이어야 하며,
...
public class L87946 {
  ...
  public static int dfs(int k, int[][] dungeons){
    ...
    for(int[] d : dungeons){
      ...
      if(a <= k){
        d[0] = 9999; // 방문표시(기준점)
        cnt = Math.max(1+dfs(k - b, dungeons) ,cnt); // 1증가
        d[0] = a; // 원상복구
      }
    }
